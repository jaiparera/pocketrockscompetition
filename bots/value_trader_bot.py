...
from bots.helpers import _current_item, legal_max_bid, _affordable, _bundle_value, _best_product_bonus_if_win
import math
# -----------------------------
# Bot 4: ValueTrader
# - Bids based on expected resource value (from partial info),
#   plus product bonus, plus rational handling of loans/investments.
...
class ValueTraderBot(PocketRocketsBot):
    ...
    def get_bid(self, obs: "GameObservation") -> "Bid":
        ...
        # (upcoming not counted in pile count, so total remaining resources is upcoming + pile)
        remaining_resources = len(obs.context.upcoming_resources) + obs.context.resource_deck_count
        late = remaining_resources <= 6
        ...
        if kind in (ActionType.AUCTION_1, ActionType.AUCTION_2):
            ...
            v += _best_product_bonus_if_win(obs, cards)
        ...
        if kind in (ActionType.INVESTMENT_5, ActionType.INVESTMENT_10):
            ...
            # but we discount early game because locking cash can lose resource opportunities.
        ...
        if kind in (ActionType.LOAN_10, ActionType.LOAN_20):
            ...
            # If late, sometimes worth 1 to buy a last resource.
